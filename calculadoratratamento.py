...
def del_invalid_chars(equation: str) -> str:
    cleaned = equation
    # Remover tudo, exceto 0123456789./*-+^e
    cleaned = re.sub(r'[^\d\.\/\*\-\+\^\(\)e]', r'', cleaned, 0)
    # Remover operadores duplicados (.+/-*^)
...
def resolve_parenteses(equation: str) -> str:
...
def calcular(equation: str) -> str:
    cleaned_equation = del_invalid_chars(equation)

    try:
        eq_parenteses_solved = resolve_parenteses(cleaned_equation)
